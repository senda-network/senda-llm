    ...
    <CommandBarProvider>
      <TooltipProvider>
        <AppHeader
          ...
          setThemeMode={vi.fn()}
          statusError={null}
          inviteWithModelCommand="closedmesh --join invite-token --model GLM-4.7-Flash-Q4_K_M"
          inviteWithModelName="GLM-4.7-Flash-Q4_K_M"
          inviteClientCommand="closedmesh --client --join invite-token"
...
describe("AppHeader", () => {
  ...
  it("shows and copies the OpenCode launcher for public meshes", async () => {
    ...
    await screen.findByText("closedmesh opencode");
    fireEvent.click(screen.getByRole("button", { name: "Copy opencode command" }));

    await waitFor(() =>
      expect(navigator.clipboard.writeText).toHaveBeenCalledWith("closedmesh opencode"),
    );
    expect(screen.getByText("closedmesh claude")).toBeInTheDocument();
    expect(screen.getByText("closedmesh goose")).toBeInTheDocument();
  });
  ...
  it("keeps private meshes focused on invite flows without OpenCode", async () => {
    ...
    await screen.findByText("closedmesh claude --join private-token");
    expect(screen.getByText("closedmesh goose --join private-token")).toBeInTheDocument();
    expect(screen.queryByText(/closedmesh opencode/i)).not.toBeInTheDocument();
    expect(
