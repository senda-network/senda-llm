...
cleanup() {
    ...
    ssh -p $REMOTE_PORT $REMOTE "pkill -f closedmesh; pkill -f rpc-server; pkill -f llama-server" 2>/dev/null || true
...
ssh -p $REMOTE_PORT $REMOTE "cd ~/bin && RUST_LOG=info nohup ./closedmesh \
