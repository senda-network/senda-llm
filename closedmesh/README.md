...
Rust implementation of closedmesh: a peer-to-peer control plane for llama.cpp inference over QUIC, with distributed routing, model orchestration, plugin hosting, and a local management API.
...
├── plugins/               built-in plugins shipped with closedmesh
...
- `closedmesh` owns the user-facing OpenAI-compatible API on `:9337`. Requests are routed by model.
...
The control plane uses protocol `closedmesh/1` with protobuf framing for mesh traffic.
...
- `GET /api/search` for read-only catalog or Hugging Face model search, returning the same JSON payload shape as `closedmesh models search --json`
...
Plugin hosting now lives in `src/plugin/` rather than a crate-root module. closedmesh supports:

- built-in plugins shipped with the binary
- external executable plugins declared in `~/.closedmesh/config.toml`
- MCP exposure through the plugin bridge

`closedmesh serve` also loads startup model config from the same file. The blackboard plugin is auto-registered unless explicitly disabled in config. Useful entry points:

```bash
closedmesh plugin list
closedmesh blackboard
closedmesh blackboard --search "routing"
closedmesh client --join <token> blackboard --mcp
```
...
`closedmesh serve` uses `~/.closedmesh/config.toml` by default, or `--config /path/to/config.toml`.
...
Bare `closedmesh serve` warns, shows help, and exits if `[[models]]` is empty.

When `[gpu].assignment = "pinned"`, every configured `[[models]]` entry must include a `gpu_id` taken from the pinnable stable IDs shown by `closedmesh gpus` / `closedmesh gpus --json`. Some fallback `stable_id` values may still be printed for inventory (`index:*`, backend-device names, and similar fallbacks), but those are not valid config identities. Pinned configs fail closed when an ID is missing, ambiguous, unsupported by the selected backend, or no longer resolves on the current host.
...
```bash
closedmesh serve --model Qwen2.5-3B --publish --mesh-name "Sydney Lab" --region AU
closedmesh discover
closedmesh discover --model GLM --region AU
...
```bash
closedmesh serve --auto --model GLM-4.7-Flash-Q4_K_M --mesh-name "poker-night"
```
