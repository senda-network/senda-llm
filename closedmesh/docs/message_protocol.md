# closedmesh Message Protocol

This document describes the wire protocol for control-plane communication between closedmesh nodes over the `meshllm.node.v1` protobuf schema on QUIC ALPN `closedmesh/1`.

## ALPN

Control-plane connections prefer ALPN `closedmesh/1`.
...
`closedmesh/1` is the only supported control-plane protocol.

- Nodes advertise `closedmesh/1` on accept.
