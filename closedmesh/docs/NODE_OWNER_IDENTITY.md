...
Today closedmesh has two separate identity concepts:
...
- `closedmesh auth init` creates an owner keypair in the keystore
...
`~/.closedmesh/key` or an ephemeral key for clients. That node identity is what
...
- **Owner identity**
  ...
  - This is the existing keystore identity created by `closedmesh auth init`
...
This is the right split for closedmesh:
...
- `closedmesh/src/crypto/ownership.rs`
  - `NodeOwnershipClaim`
  - `SignedNodeOwnership`
  - canonical serialization for signing
  - sign/verify helpers
- `closedmesh/src/crypto/mod.rs`
...
```text
closedmesh-node-ownership-v1
```

### `closedmesh/src/runtime/`
...
```bash
closedmesh auth renew-node
closedmesh auth rotate-node
closedmesh auth rotate-node --revoke-current
closedmesh auth rotate-owner
```
...
```bash
closedmesh --owner-key ~/.closedmesh/owner-keystore.json
closedmesh --owner-required
closedmesh --owner-label studio-mac
closedmesh --trust-owner <owner_id>
closedmesh --trust-policy off|prefer-owned|require-owned|allowlist
```
...
```bash
closedmesh auth init
closedmesh auth status
closedmesh auth renew-node
closedmesh auth sign-node --node-key ~/.closedmesh/key --out ~/.closedmesh/node-ownership.json
closedmesh auth verify-node --file ~/.closedmesh/node-ownership.json
closedmesh auth rotate-node
closedmesh auth rotate-node --revoke-current
closedmesh auth revoke-owner <owner_id>
closedmesh auth revoke-node --cert-id <cert_id>
closedmesh auth revoke-node --node-id <node_endpoint_id>
closedmesh auth rotate-owner
closedmesh auth trust add <owner_id>
closedmesh auth trust list
closedmesh auth trust remove <owner_id>
```
...
closedmesh --model Qwen3-14B --owner-key ~/.closedmesh/owner-keystore.json --owner-label studio

closedmesh --join <TOKEN> --owner-key ~/.closedmesh/owner-keystore.json --owner-label mini
```

Strict private mesh:

```bash
closedmesh --model Qwen3-30B \
  --owner-key ~/.closedmesh/owner-keystore.json \
...
- `~/.closedmesh/trusted-owners.json`
...
That gives closedmesh a durable answer to:
