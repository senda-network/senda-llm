...
```bash
closedmesh benchmark import-prompts \
  --source mt-bench \
  --limit 8 \
  --max-tokens 256 \
...
```bash
closedmesh benchmark moe-model-matrix \
...
```bash
closedmesh benchmark moe-grouping --model /path/to/model.gguf --nodes 2
closedmesh benchmark moe-micro-analyze --model /path/to/model.gguf --prompts evals/moe/prompts/mt-bench-8.jsonl
```
...
```bash
closedmesh --model /path/to/model.gguf --split \
  --moe-ranking analyze \
