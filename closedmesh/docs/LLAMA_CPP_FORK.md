...
closedmesh builds llama.cpp from upstream `ggml-org/llama.cpp` plus a local patch
...
closedmesh still launches `rpc-server` and `llama-server`.
...
the `CLOSEDMESH_LLAMA_DIR` override.
