...
Callback hooks from llama-server into closedmesh during inference.

Related: [#183](https://github.com/michaelneale/closedmesh/issues/183), [#165](https://github.com/michaelneale/closedmesh/issues/165), [PR #225](https://github.com/michaelneale/closedmesh/pull/225)

---

## What it does

llama-server detects when it might need help and calls closedmesh on localhost. closedmesh consults other models in the mesh and replies with context to inject. The caller sees one seamless response.
...
**What closedmesh does:** Extracts the image URL from `mesh_image_url` in messages, finds a vision-capable peer, sends the image for captioning, returns the caption for injection into the prompt.
...
**What closedmesh does:** Finds up to 2 peers serving different models, races them for a second opinion, injects the winner's answer with "Reference answer" framing.
...
When a hook fires, closedmesh consults other models on the mesh:
...
### Rust side (closedmesh, `micn/virtual-llm` branch)
...
C++ source files are copied into `closedmesh/llama-patches/` so both C++ and Rust changes live in one repo / one PR. `sync.sh` copies them into `llama.cpp/` before building.
