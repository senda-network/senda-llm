...
- Support multimodal inference through closedmesh using llama.cpp-compatible models.
...
Implement as a closedmesh compatibility shim after chat completions are solid.
...
This is the current closedmesh capability matrix, based on the inference logic in
`closedmesh/src/models/capabilities.rs`.

Important:

- `supported` means closedmesh will treat the family/model as runtime-capable for routing and UI.
- `likely` means closedmesh will surface a weaker multimodal hint, but not rely on it as a hard runtime capability.
...
| Family / signal | closedmesh status | Notes / examples |
|---|---|---|
...
closedmesh should detect multimodal intent from structured content blocks, not just text keywords.
...
Some model families support video at the model level, but that should not be treated as available in closedmesh yet.

Current working assumption:

- open multimodal models with video support exist
- the current llama.cpp integration path in closedmesh should be treated as image/audio only
