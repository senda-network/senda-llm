...
See [ROADMAP.md](../../ROADMAP.md) for how this fits into closedmesh.

## What's Implemented

All core phases are complete and integrated into closedmesh.
...
- **Cached rankings**: local analysis results remain in the closedmesh cache and are preferred when they are at least as strong as published data. Published rankings only replace local cache when they are stronger.
- **Runtime resolution**: `closedmesh moe plan` and `serve` check the local closedmesh cache first, then the Hugging Face dataset, and prefer `full-*` over `micro-*`.
- **HF cache behavior**: dataset artifacts stay in the Hugging Face cache when downloaded; they are not copied into `~/.cache/closedmesh`.
...
- `closedmesh moe plan <model>` resolves rankings from local cache or `meshllm/moe-rankings` and produces a placement recommendation.
- `closedmesh moe analyze full <model>` and `closedmesh moe analyze micro <model>` generate local ranking artifacts.
- `closedmesh moe share <model>` opens a contribution PR against `meshllm/moe-rankings` when a locally generated ranking is new.
...
- `run_split()` calls `llama-moe-split` to produce per-node GGUFs (trunk + expert subset). Cached at `~/.cache/closedmesh/splits/<model>/<n>-nodes/node-<i>.gguf`.
...
- GLM-4.7-Flash-Q4_K_M: MoE auto-detected (64 experts, top-4), fits locally → solo mode, no split. If split, closedmesh now prefers cached or freshly computed analysis over the sequential fallback.
...
This should give closedmesh the following MoE behavior:
...
As closedmesh moves to canonical per-model descriptors in the gossip protocol, MoE should evolve to consume `ModelTopology` from those descriptors instead of treating MoE as a GGUF-only local concern.
...
- when a model is detected as MoE but only has fallback topology, closedmesh can still run conservatively
- if `llama-moe-analyze` is available, closedmesh can run it in the background for that exact model revision
