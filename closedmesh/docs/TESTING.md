...
```bash
closedmesh gpus
closedmesh gpus --json | jq .
closedmesh gpu benchmark --json | jq .
```
...
- If `[[models]]` is empty, `closedmesh serve` should print a `⚠️` warning, show help, and exit cleanly
...
Then create `~/.closedmesh/config.toml` with a real pinnable stable ID from that output (for example `pci:*`, `uuid:*`, or `metal:*`, not fallback IDs like `index:*` or backend-device names):
...
- Startup should succeed only when `gpu_id` matches a valid local pinnable stable ID from `closedmesh gpus`
- If the pinned ID is missing, ambiguous, unsupported, or stale, startup should fail closed before local launch
- Explicit `closedmesh serve --model ...` should still bypass configured `[[models]]` and therefore bypass config-owned pinned IDs
...
|---|---|---|
| Plain terminal | `closedmesh serve --model Qwen2.5-3B` | Dashboard renders, resizes cleanly, `h`/`i`/`q` work, and exit restores the prompt. |
| Piped stdin | `true | closedmesh serve --model Qwen2.5-3B` | No line reader is spawned; pretty output stays line-oriented. |
| Unsupported terminal | `TERM=dumb closedmesh serve --model Qwen2.5-3B` | Dashboard is disabled and pretty output uses fallback lines. |
| tmux, mouse off | `tmux new 'closedmesh serve --model Qwen2.5-3B'` | Dashboard renders and exits cleanly; keyboard navigation works. |
| tmux, mouse on | Inside tmux: `set -g mouse on`, then run closedmesh | Wheel events page the dashboard instead of disappearing into tmux history. |
| GNU screen default | `screen closedmesh serve --model Qwen2.5-3B` | If the alternate screen is unavailable, fallback behavior or clean restoration is acceptable. |
| GNU screen altscreen | In `~/.screenrc`: `altscreen on`, then run closedmesh | Dashboard enters/leaves the alternate screen cleanly. |
...
```bash
closedmesh serve --model Qwen2.5-3B --console
```
...
```bash
closedmesh serve --model Qwen2.5-3B --headless --console 3131
```
...
closedmesh serve --model Qwen2.5-32B --bind-port 7842
# node B (joins)
closedmesh serve --model Qwen2.5-32B --join <TOKEN>
```
...
```bash
# host with --split
closedmesh serve --model Qwen2.5-32B --bind-port 7842 --split
# worker joins
closedmesh serve --model Qwen2.5-32B --join <TOKEN>
```
...
```bash
closedmesh client --join <TOKEN> --port 9555
```
...
closedmesh serve --model Qwen2.5-3B --model GLM-4.7-Flash --console
...
closedmesh serve --model Qwen2.5-3B --model GLM-4.7-Flash
...
```bash
closedmesh unload GLM-4.7-Flash-Q4_K_M
```
...
```bash
# Running node
closedmesh serve --model Qwen2.5-0.5B-Instruct-Q4_K_M --console
...
- `closedmesh status` shows the local models currently backed by running inference processes, including PID when present
...
- Unloading any local model removes it cleanly without terminating the closedmesh process
...
closedmesh serve --model Qwen2.5-3B --mesh-name "test-mesh"
```

- Log: `📌 Mesh ID: <hex>`
- `~/.closedmesh/last-mesh` contains the same hex
...
closedmesh serve --model Qwen2.5-3B --mesh-name "test-mesh"
# Joiner
closedmesh serve --model Qwen2.5-3B --join <TOKEN>
```

- Joiner log: `📌 Mesh ID: <same hex as originator>`
- Joiner's `~/.closedmesh/last-mesh` matches originator's mesh ID
- Mesh ID arrives via gossip (worker nodes) or routing table (passive clients)

### 18. Sticky mesh preference

- Join a mesh → `~/.closedmesh/last-mesh` saved
...
closedmesh serve --model Qwen2.5-3B --port 8090
# Joiner
closedmesh serve --model Qwen2.5-3B --join <TOKEN> --port 8091
```
...
```bash
closedmesh serve --model Qwen2.5-3B
```
...
- Prints the same usage/help text as `closedmesh --help`
- No ports are bound
...
```bash
# node A
closedmesh --model Qwen3-Coder-Next-Q4_K_M --auto --no-self-update

# node B
closedmesh --model Qwen3-Coder-Next-Q4_K_M --auto --no-self-update --split --join <TOKEN>
```
...
The control plane uses QUIC ALPN `closedmesh/1` with the `meshllm.node.v1` protobuf schema. All five scoped control-plane streams use 4-byte LE framing followed by protobuf bytes.
...
Set `CLOSEDMESH_EPHEMERAL_KEY=1` to give a second process a unique identity on the same machine.
Without this, both processes share `~/.closedmesh/key` and appear as the same node.

### 14. Forced split on one machine

```bash
# Terminal 1: host with --split
closedmesh serve --model Qwen2.5-3B --port 9337 --split --console

# Terminal 2: worker with ephemeral key
CLOSEDMESH_EPHEMERAL_KEY=1 closedmesh serve --model Qwen2.5-3B --join <TOKEN> --port 9338 --split --max-vram 1
```
...
```bash
# Terminal 1: host
closedmesh serve --model Qwen2.5-3B --port 9337

# Terminal 2: client (the client surface uses an ephemeral key automatically)
closedmesh client --join <TOKEN> --port 9338
```
...
just bundle
# scp, then on remote:
codesign -s - ~/mesh-bundle/closedmesh ~/mesh-bundle/rpc-server ~/mesh-bundle/llama-server
xattr -cr ~/mesh-bundle/closedmesh ~/mesh-bundle/rpc-server ~/mesh-bundle/llama-server
```
