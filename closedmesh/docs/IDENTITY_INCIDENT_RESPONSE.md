...
```bash
closedmesh auth revoke-node --cert-id <old-cert-id>
closedmesh auth revoke-node --node-id <old-node-id>   # optional emergency block
...
```bash
closedmesh auth revoke-owner <old-owner-id>
closedmesh auth rotate-owner
