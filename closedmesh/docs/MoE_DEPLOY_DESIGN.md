...
This documents how MoE expert sharding is implemented in closedmesh. Originally a design doc; updated to reflect the actual implementation.

## User Experience

```bash
# MoE auto-detected — splits if needed, runs solo if it fits
closedmesh serve --model Qwen3-30B-A3B-Q4_K_M

# Hidden test/debug override: force splitting even if the model fits locally
closedmesh serve --model Qwen3-30B-A3B-Q4_K_M --split
```
...
Normal operators should use plain `closedmesh --model ...` or `--auto`.
...
Published rankings are downloaded through the normal Hugging Face cache and remain there. closedmesh does not copy dataset artifacts into its own cache.
...
Splits are cached at `~/.cache/closedmesh/splits/<model>/<n>-nodes/node-<i>.gguf`. Invalidated implicitly when node count changes (different directory).
...
As closedmesh moves toward protocol-level `ServedModelDescriptor` objects, MoE should stop depending only on local GGUF inspection and instead consume `ModelTopology.moe` when it is available.
...
- use `closedmesh moe analyze full` or `closedmesh moe analyze micro` to generate local rankings
- use `closedmesh moe share` to open contribution PRs against `meshllm/moe-rankings`
- when `serve` had to generate a local ranking because no published one was available, it should suggest `closedmesh moe share <model>`
