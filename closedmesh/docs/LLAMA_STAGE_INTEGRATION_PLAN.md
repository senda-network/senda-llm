...
`/Users/jdumay/code/llama-stage-runtime` into closedmesh.
...
- add an in-process llama backend for closedmesh, eventually replacing the
...
closedmesh's historical baseline depended on a pinned Mesh-LLM llama.cpp fork.
...
- closedmesh proxies local and remote requests to those HTTP servers
...
replacement for `llama-server`. closedmesh currently relies on behavior owned by
...
`GGML_CUDA_FA_ALL_QUANTS=ON`, because closedmesh intentionally uses asymmetric KV
...
folding it into unrelated closedmesh modules. That keeps a later move from
...
closedmesh can then depend on `llama-stage-*` crates by workspace path.
...
Introduce an inference backend abstraction owned by `closedmesh/src/inference/`.
...
For early integration, keep closedmesh's routing and tunneling model stable:
...
- `cargo check -p closedmesh` passes
...
- expose health/readiness to closedmesh's existing state machinery
...
future experimental:
  closedmesh embedded stage -> QUIC stage transport -> closedmesh embedded stage
```
...
not currently provide all llama-server product behavior. closedmesh will need a
...
- `cargo check -p closedmesh` succeeds
