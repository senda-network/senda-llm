...
#
# If no model path given, downloads Qwen3-8B-Q4_K_M via closedmesh.
set -e
...
if [ -z "$MODEL" ] || [ ! -f "$MODEL" ]; then
    ...
    echo "Downloading $MODEL_STEM via closedmesh..."
    set +e
...
if [ -z "${MODEL:-}" ] || [ ! -f "$MODEL" ]; then
    ...
    echo "Or download first: closedmesh models download $MODEL_STEM"
