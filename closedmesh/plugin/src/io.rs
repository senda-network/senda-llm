...
pub async fn connect_from_env() -> Result<LocalStream> {
    let endpoint = std::env::var("CLOSEDMESH_PLUGIN_ENDPOINT")
        .context("CLOSEDMESH_PLUGIN_ENDPOINT is not set for plugin process")?;
    let transport =
        std::env::var("CLOSEDMESH_PLUGIN_TRANSPORT").unwrap_or_else(|_| default_transport().into());
