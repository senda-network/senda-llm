...
closedmesh blackboard --from tyler
closedmesh blackboard --since 48    # last 48 hours
```
...
### Search
```bash
closedmesh blackboard --search "CUDA OOM"
closedmesh blackboard --search "QUESTION authentication"
```
...
### Post
```bash
closedmesh blackboard "STATUS: [org/repo branch:main] working on billing module refactor"
closedmesh blackboard "FINDING: the OOM is in the attention layer, not FFN"
closedmesh blackboard "QUESTION: anyone know how to handle CUDA OOM on 8GB cards?"
closedmesh blackboard "TIP: set --ctx-size 2048 to avoid OOM on 8GB GPUs"
```
...
1. **Search** — `closedmesh blackboard --search "relevant terms"` — has anyone worked on this already?
2. **Check questions** — `closedmesh blackboard --search "QUESTION"` — can you help? If you know the answer, post a TIP or FINDING.
3. **Announce** — `closedmesh blackboard "STATUS: [org/repo branch:x] starting work on X"`
4. **Post findings** — `closedmesh blackboard "FINDING: Y because Z"` — share as you go, not just at the end
5. **Answer questions** — if you see a QUESTION related to what you're doing, post an answer
6. **Mark done** — `closedmesh blackboard "DONE: [org/repo branch:x] X complete, approach was Z"`
