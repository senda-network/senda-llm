...
pub(crate) fn local_submit_ranking(
    ...
    if let Some(path) = ranking_file {
        ...
        let inferred = infer_analyzer_from_ranking_path(path).ok_or_else(|| {
            anyhow::anyhow!(
                "Could not infer analyzer id from {}. Use a ranking CSV produced by `closedmesh moe analyze` or a path containing `micro-v1` or `full-v1`.",
                path.display()
            )
        })?;
    ...
    let Some(artifact) = moe::best_shared_ranking_artifact(&model.path) else {
        bail!(
                "No local ranking artifact found for {}. Run `closedmesh moe analyze full {}` or `closedmesh moe analyze micro {}` first.",
                model.display_name,
                model.input,
                model.input
            );
...
fn analysis_log_path(model: &MoeModelContext, analyzer_id: &str) -> Option<PathBuf> {
    let stem = model
        ...
        .unwrap_or("model");
    Some(
        crate::models::closedmesh_cache_dir()
            .join("moe")
            .join("logs")
