...
impl SearchFormatter for ConsoleFormatter {
    ...
    fn render_hf_results(
        ...
        for (index, result) in results.iter().enumerate() {
            ...
            writeln!(
                &mut output,
                "   show: closedmesh models show {}",
                result.exact_ref
            )?;
            writeln!(
                &mut output,
                "   download: closedmesh models download {}",
                result.exact_ref
            )?;
            if let Some(size) = &result.size_label {
...
impl ModelsFormatter for ConsoleFormatter {
    ...
    fn render_installed(&self, rows: &[InstalledRow]) -> Result<()> {
        ...
        writeln!(
            &mut output,
            "🗑️ Delete example: closedmesh models delete {}",
            rows[0].model_ref
        )?;
        writeln!(&mut output)?;
        for row in rows {
            ...
            writeln!(
                &mut output,
                "   delete: closedmesh models delete {}",
                row.model_ref
            )?;
    ...
    fn render_show(&self, details: &ModelDetails, variants: Option<&[ModelDetails]>) -> Result<()> {
        ...
        println!("📥 Download:");
        if model_kind_code(details.kind) == "mlx" {
            println!("   closedmesh models download {}", details.exact_ref);
        } else {
