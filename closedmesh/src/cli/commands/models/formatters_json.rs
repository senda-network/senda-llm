...
impl ModelsFormatter for JsonFormatter {
        ...
        let results: Vec<Value> = models
            ...
            .map(|model| {
                ...
                json!({
                    ...
                    "ref": model.name,
                    "show": format!("closedmesh models show {}", model.name),
                    "download": format!("closedmesh models download {}", model.name),
    ...
    fn render_installed(&self, rows: &[InstalledRow]) -> Result<()> {
        let models: Vec<Value> = rows
            ...
            .map(|row| {
                json!({
                    ...
                    "ref": row.model_ref,
                    "show": format!("closedmesh models show {}", row.model_ref),
                    "download": format!("closedmesh models download {}", row.model_ref),
                    "delete": format!("closedmesh models delete {}", row.model_ref),
                    "path": row.path,
        ...
        print_json(json!({
            ...
            "delete_example": rows
                .first()
                .map(|row| format!("closedmesh models delete {}", row.model_ref)),
            "results": models,
        }))
    }
