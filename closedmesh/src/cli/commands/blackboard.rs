...
pub(crate) async fn run_blackboard(
    ...
    if status_resp.is_err() {
        eprintln!("No closedmesh node running on port {port}.");
        eprintln!();
        eprintln!("Blackboard requires a running mesh node:");
        eprintln!("  Private mesh:  closedmesh client  (share the join token printed out)");
        eprintln!("  Join a mesh:   closedmesh client --join <token>");
        eprintln!("  Public mesh:   closedmesh client --auto");
        eprintln!();
        eprintln!("See https://github.com/Mesh-LLM/closedmesh for setup guide.");
        std::process::exit(1);
    }
    ...
    if let Some(msg) = text {
        ...
        let resp = client
            ...
            .send()
            .await
            .context("Cannot reach closedmesh — is it running?")?;
...
pub(crate) fn install_skill() -> Result<()> {
    ...
    eprintln!(
        "   Make sure closedmesh is running and the blackboard plugin is not disabled in config."
    );
    Ok(())
}
