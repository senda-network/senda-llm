...
pub(crate) async fn run_discover(
    ...
    eprintln!("🔍 Searching Nostr relays for closedmesh meshes...");
    ...
    if auto_join {
        ...
        eprintln!("\nRun:");
        eprintln!("  closedmesh --join {}", best.listing.invite_token);
        println!("{}", best.listing.invite_token);
    } else {
        eprintln!("To join a mesh:");
        eprintln!("  closedmesh --join <token>");
        eprintln!("\nOr use `closedmesh discover --join` to auto-join the best match.");
    }
...
/// Stop all closedmesh instances and their child servers.
