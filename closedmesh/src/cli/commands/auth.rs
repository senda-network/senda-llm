...
pub(crate) fn run_init(
    ...
    eprintln!("- Start closedmesh and it will automatically attest nodes from this keystore.");
    if custom_owner_key {
        eprintln!(
            "- Pass --owner-key {} when starting closedmesh.",
            path.display()
        );
    }
...
pub(crate) fn run_status(
    ...
    if !keystore_exists(&owner_key_path) {
        ...
        eprintln!("Run `closedmesh auth init` to create one.");
    } else {
    ...
    |owner_key,
     ...
     trust_store| {
        ...
        if !owner_key_path.exists() {
            eprintln!("No owner keystore found at {}", owner_key_path.display());
            eprintln!(
                "Run `closedmesh auth init` or `closedmesh auth sign-node` later to attest this node."
            );
            return Ok(());
        }
...
mod tests {
    ...
    fn init_defaults_to_keychain_then_load_round_trip() {
        ...
        let dir =
            std::env::temp_dir().join(format!("closedmesh-keychain-rt-{}", rand::random::<u64>()));
