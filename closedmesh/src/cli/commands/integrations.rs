...
fn build_opencode_launch_spec_with_limits(
    ...
    mesh_provider.insert("name".to_string(), serde_json::json!("closedmesh"));
...
fn opencode_missing_binary_guidance(
    ...
    vec![
        ...
        "Then rerun through closedmesh:".to_string(),
        format!("  closedmesh opencode --host {host} --model {chosen}"),
        "closedmesh injects OPENCODE_CONFIG_CONTENT automatically when launching OpenCode."
            .to_string(),
    ]
}
...
fn cleanup_mesh_child(mesh_child: &mut Option<std::process::Child>) {
    if let Some(ref mut child) = mesh_child {
        eprintln!("🧹 Stopping closedmesh node we started...");
        let _ = child.kill();
        let _ = child.wait();
    }
}
...
pub(crate) async fn run_goose(model: Option<String>, port: u16) -> Result<()> {
    ...
    let provider = serde_json::json!({
        "name": "mesh",
        "engine": "openai",
        "display_name": "closedmesh",
        "description": "Distributed LLM inference via closedmesh",
        "api_key_env": "",
    ...
    if goose_app.exists() {
        ...
        if mesh_child.is_some() {
            eprintln!(
                "ℹ️  closedmesh node running in background (kill manually or use `closedmesh stop`)"
            );
        }
    } else {
...
fn resolve_opencode_config_path_from_home(
    ...
    if jsonc_path.exists() {
        anyhow::bail!(
            "Found {} but closedmesh only writes opencode.json. Rename or migrate it to {} and rerun `closedmesh opencode --write`.",
            jsonc_path.display(),
            json_path.display()
        );
    }
...
mod tests {
    ...
    fn opencode_launch_spec_uses_mesh_provider_and_v1_base_url() {
        ...
        assert_eq!(config["provider"]["mesh"]["name"], "closedmesh");
        assert_eq!(
    ...
    fn opencode_missing_binary_reports_official_install_hint() {
        ...
        assert_eq!(lines[2], "Then rerun through closedmesh:");
        assert_eq!(
            lines[3],
            "  closedmesh opencode --host 127.0.0.1:9337 --model GLM-4.7-Flash-Q4_K_M"
        );
        assert_eq!(
            lines[4],
            "closedmesh injects OPENCODE_CONFIG_CONTENT automatically when launching OpenCode."
    ...
    fn test_write_overwrites_mesh_provider() {
        ...
        assert_eq!(
            parsed["provider"]["mesh"]["name"], "closedmesh",
