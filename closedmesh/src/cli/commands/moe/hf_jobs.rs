    ...
    fn analyze_command_with_env_ref(&self) -> String {
        match self {
            Self::Full {
                ...
                "./closedmesh moe analyze full \"$MODEL_REF\" --context-size {} --n-gpu-layers {}",
            ...
            Self::Micro {
                ...
                "./closedmesh moe analyze micro \"$MODEL_REF\" --prompt-count {} --token-count {} --context-size {} --n-gpu-layers {}",
                prompt_count,
                token_count,
                context_size,
                n_gpu_layers
...
async fn submit_job(spec: JobSubmissionSpec) -> Result<()> {
    ...
    let payload = json!({
        ...
        "arguments": [],
        "environment": {
            "CLOSEDMESH_RELEASE_URL": release_url,
...
fn remote_bash_script(spec: &JobSubmissionSpec) -> String {
    ...
    let share_command =
        "./closedmesh moe share \"$MODEL_REF\" --dataset-repo \"$DATASET_REPO\"".to_string();
