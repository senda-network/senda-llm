...
mod tests {
    ...
    fn normalize_runtime_surface_args_rewrites_serve_invocation() {
        ...
        assert_eq!(
            ...
            vec!["closedmesh", "--auto", "--model", "Qwen3-8B-Q4_K_M"]
                .into_iter()
                .map(OsString::from)
                .collect::<Vec<_>>()
        );
    ...
    fn normalize_runtime_surface_args_bare_serve_rewrites_to_help() {
        let normalized = normalize_runtime_surface_args(["closedmesh", "serve"]);
    ...
    fn normalize_runtime_surface_args_rewrites_client_invocation() {
        let normalized =
            normalize_runtime_surface_args(["closedmesh", "client", "--auto", "--port", "9337"]);

        assert_eq!(normalized.explicit_surface, Some(RuntimeSurface::Client));
        assert_eq!(
            normalized.normalized,
            vec!["closedmesh", "--client", "--auto", "--port", "9337"]
                .into_iter()
                .map(OsString::from)
                .collect::<Vec<_>>()
        );
    ...
    fn normalize_runtime_surface_args_keeps_non_runtime_subcommands() {
        let normalized = normalize_runtime_surface_args(["closedmesh", "download", "foo"]);

        assert_eq!(normalized.explicit_surface, None);
        assert_eq!(
            normalized.normalized,
            vec!["closedmesh", "download", "foo"]
                .into_iter()
                .map(OsString::from)
                .collect::<Vec<_>>()
        );
    ...
    fn legacy_runtime_surface_warning_for_top_level_serve_flags() {
        let normalized =
            normalize_runtime_surface_args(["closedmesh", "--auto", "--model", "Qwen3-8B-Q4_K_M"]);
        ...
        assert!(warning.contains("closedmesh serve --auto --model Qwen3-8B-Q4_K_M"));
    }

    #[test]
    fn legacy_runtime_surface_warning_for_top_level_client_flag() {
        let normalized = normalize_runtime_surface_args(["closedmesh", "--auto", "--client"]);
        let cli = Cli::parse_from(normalized.normalized.clone());
        ...
        assert!(warning.contains("closedmesh client --auto"));
    }

    #[test]
    fn explicit_runtime_surface_suppresses_legacy_warning() {
        let normalized = normalize_runtime_surface_args(["closedmesh", "client", "--auto"]);
        let cli = Cli::parse_from(normalized.normalized.clone());
    ...
    fn auth_status_accepts_owner_key_locally() {
        let cli = Cli::parse_from(["closedmesh", "auth", "status", "--owner-key", "owner.json"]);
    ...
    fn auth_status_rejects_runtime_only_owner_required_flag() {
        let err = Cli::try_parse_from(["closedmesh", "auth", "status", "--owner-required"])
    ...
    fn gpus_command_parses_without_subcommand() {
        let cli = Cli::parse_from(["closedmesh", "gpus"]);
    ...
    fn gpu_alias_parses_without_subcommand() {
        let cli = Cli::parse_from(["closedmesh", "gpu"]);
    ...
    fn gpus_command_accepts_json_flag() {
        let cli = Cli::parse_from(["closedmesh", "gpus", "--json"]);
    ...
    fn gpu_benchmark_subcommand_parses() {
        let cli = Cli::parse_from(["closedmesh", "gpu", "benchmark"]);
    ...
    fn gpu_benchmark_subcommand_accepts_json_flag() {
        let cli = Cli::parse_from(["closedmesh", "gpu", "benchmark", "--json"]);
    ...
    fn cli_accepts_headless_flag_for_serve_surface() {
        let args = vec!["closedmesh", "serve", "--headless", "--auto"];
    ...
    fn cli_accepts_headless_flag_for_client_surface() {
        let args = vec!["closedmesh", "client", "--headless", "--auto"];
        let normalized = normalize_runtime_surface_args(args);
        let cli = Cli::try_parse_from(&normalized.normalized).unwrap();
        assert!(cli.headless);
    }
    ...
    fn legacy_no_console_remains_ignored_in_headless_tests() {
        let args = vec!["closedmesh", "serve", "--no-console"];
        let normalized = normalize_runtime_surface_args(args);
        let cli = Cli::try_parse_from(&normalized.normalized).unwrap();
        assert!(
            !cli.headless,
    ...
    fn opencode_command_rejects_port_flag() {
        let err = Cli::try_parse_from(["closedmesh", "opencode", "--port", "9337"])
    ...
    fn cli_defaults_log_format_to_pretty() {
        let normalized = normalize_runtime_surface_args(["closedmesh", "serve", "--auto"]);
    ...
    fn cli_rejects_invalid_log_format_values() {
        let err = Cli::try_parse_from(["closedmesh", "--log-format", "invalid"])
    ...
    fn cli_log_format_selection_is_independent_across_runs() {
        let pretty = Cli::parse_from(["closedmesh", "--log-format", "pretty"]);
        assert_eq!(pretty.log_format, LogFormat::Pretty);

        let json = Cli::parse_from(["closedmesh", "--log-format", "json"]);
        assert_eq!(json.log_format, LogFormat::Json);

        let pretty_again = Cli::parse_from(["closedmesh", "--log-format", "pretty"]);
        assert_eq!(pretty_again.log_format, LogFormat::Pretty);

        let json_again = Cli::parse_from(["closedmesh", "--log-format", "json"]);
    ...
    fn cli_accepts_private_only_flag() {
        let normalized = normalize_runtime_surface_args(["closedmesh", "serve", "--private-only"]);
        let cli = Cli::parse_from(normalized.normalized);
    ...
    fn cli_private_only_conflicts_with_auto() {
        let normalized =
            normalize_runtime_surface_args(["closedmesh", "serve", "--private-only", "--auto"]);
        let err =
    ...
    fn cli_private_only_conflicts_with_publish() {
        let normalized =
            normalize_runtime_surface_args(["closedmesh", "serve", "--private-only", "--publish"]);
