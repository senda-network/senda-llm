    ...
    fn message(&self) -> String {
        match self {
            ...
            OutputEvent::Startup { message, .. } => message
                .clone()
                .unwrap_or_else(|| "closedmesh starting".to_string()),
            ...
            OutputEvent::RuntimeReady { .. } => "closedmesh runtime ready".to_string(),
            ...
            OutputEvent::Shutdown { reason } => reason
                .clone()
                .unwrap_or_else(|| "closedmesh shutting down".to_string()),
        }
    }
...
fn render_dashboard_text(state: &DashboardState) -> String {
    ...
    let mut header = String::from("closedmesh");
...
fn startup_progress_event(event: &OutputEvent) -> Option<(Option<String>, String)> {
    match event {
        OutputEvent::Startup { version, .. } => Some((
            ...
            format!("starting closedmesh {version}"),
        )),
        ...
        OutputEvent::RuntimeReady { .. } => Some((
            ...
            "closedmesh runtime ready".to_string(),
        )),
        _ => None,
    }
}
...
fn format_tui_panel_title(state: &DashboardState, panel: DashboardPanel) -> String {
    ...
    match panel {
        ...
        DashboardPanel::Webserver => format!("{focus_marker} closedmesh Processes"),
...
mod tests {
    ...
    fn sample_events_covering_all_variants() -> Vec<OutputEvent> {
        vec![
            ...
            OutputEvent::Startup {
                ...
                message: Some("closedmesh starting".to_string()),
            },
    ...
    fn tui_process_tables_render_empty_states_without_collapsing() {
        ...
        assert!(rendered.contains("closedmesh"));
    ...
    fn tui_process_tables_render_headers_and_joined_model_metadata() {
        ...
        assert!(rendered.contains("closedmesh Processes"));
    }
    ...
    fn dashboard_formatter_renders_warning_context_in_mesh_history() {
        ...
        let dashboard = formatter
            .format(&OutputEvent::Warning {
                message: "⚠️ top-level --client now maps to `closedmesh client`; re-running with client semantics"
                    .to_string(),
                context: None,
            })
        ...
        assert!(dashboard.contains(
            "top-level --client now maps to `closedmesh client`; re-running with client semantics"
        ));
        assert!(!dashboard.contains(
            "⚠️ ⚠️ top-level --client now maps to `closedmesh client`; re-running with client semantics"
        ));
    }
    ...
    fn dashboard_formatter_keeps_pinned_sections_and_bounds_mesh_history() {
        ...
        assert!(!dashboard.contains("closedmesh starting"));
    }
