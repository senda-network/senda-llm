...
pub(crate) enum MoeCommand {
    ...
    Plan {
        ...
        #[arg(long)]
        max_vram: Option<f64>,
        /// Optional node count override. When omitted, closedmesh recommends a minimum node count.
        #[arg(long)]
        nodes: Option<usize>,
    ...
    /// Share a local ranking artifact with other closedmesh users via the canonical Hugging Face dataset.
    Share {
        /// Model spec: local path, catalog name, HF exact ref, HF repo selector like `org/repo:BF16@main`, or HF URL.
        model: String,
        /// Override the ranking CSV path instead of resolving a local cached artifact.
        /// This should point to a ranking CSV, such as a file produced by `closedmesh moe analyze`.
        #[arg(long)]
        ranking_file: Option<PathBuf>,
...
pub(crate) enum MoeAnalyzeCommand {
    ...
    Full {
        /// Model spec: local path, catalog name, HF exact ref, HF repo selector like `org/repo:BF16@main`, or HF URL.
        model: String,
        /// Automatically run `closedmesh moe share` after a successful local analysis.
        #[arg(long)]
        share: bool,
...
pub(crate) struct HfJobArgs {
    ...
    /// GitHub repo that hosts the closedmesh release bundle used by the remote job.
    #[arg(long, default_value = "Mesh-LLM/closedmesh")]
