...
pub fn closedmesh_cache_dir() -> PathBuf {
    dirs::cache_dir()
        .unwrap_or_else(|| {
            dirs::home_dir()
...
pub fn model_metadata_cache_dir() -> PathBuf {
    closedmesh_cache_dir().join("model-meta")
}
...
mod tests {
    ...
    fn huggingface_cache_prefers_explicit_hub_cache() {
        ...
        std::env::set_var("HF_HUB_CACHE", "/tmp/closedmesh-hub-cache");
        std::env::set_var("HF_HOME", "/tmp/closedmesh-hf-home");
        std::env::set_var("XDG_CACHE_HOME", "/tmp/closedmesh-xdg");

        assert_eq!(
            huggingface_hub_cache_dir(),
            PathBuf::from("/tmp/closedmesh-hub-cache")
        );
    ...
    fn huggingface_cache_falls_back_to_hf_home() {
        ...
        assert_eq!(
            huggingface_hub_cache_dir(),
            PathBuf::from("/tmp/closedmesh-hf-home").join("hub")
        );
    ...
    fn mmproj_path_prefers_bf16_generic_precision_variants() {
        let temp = std::env::temp_dir().join(format!(
            "closedmesh-mmproj-precision-test-{}",
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
                .as_nanos()
    ...
    fn resolve_mmproj_path_prefers_explicit_override() {
        let temp = std::env::temp_dir().join(format!(
            "closedmesh-mmproj-override-test-{}",
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
                .as_nanos()
    ...
    fn mmproj_path_prefers_quant_matched_named_candidate() {
        ...
        let temp = std::env::temp_dir().join(format!(
            "closedmesh-mmproj-quant-named-test-{}",
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
                .as_nanos()
    ...
    fn mmproj_path_prefers_quant_matched_generic_sibling() {
        ...
        let temp = std::env::temp_dir().join(format!(
            "closedmesh-mmproj-quant-sibling-test-{}",
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
                .as_nanos()
