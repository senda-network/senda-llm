    ...
    let payload_results: Vec<Value> = results
        ...
        .map(|result| {
            json!({
                ...
                "show": format!("closedmesh models show {}", result.exact_ref),
                "download": format!("closedmesh models download {}", result.exact_ref),
...
mod tests {
    ...
    fn search_catalog_json_payload_uses_cli_contract_fields() {
        ...
        assert_eq!(
            result["show"],
            serde_json::json!("closedmesh models show Qwen3-Coder-Next-Q4_K_M")
        );
    }
