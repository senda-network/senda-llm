...
/// Default management/API port a closedmesh runtime listens on. Matches
        ...
        if path.exists() {
            ...
            if let Err(err) = std::fs::remove_file(path) {
                if is_file_in_use_error(&err) {
                    bail!(
                        "Cannot remove {}: file is still in use by a closedmesh runtime. \
                         Stop the service (`closedmesh service stop`) or unload the model \
                         (`closedmesh runtime models drop {}`) and retry. (os error {})",
                        path.display(),
                        identifier,
...
fn ensure_delete_path_allowed(path: &Path) -> Result<()> {
    ...
    let mesh_root = normalize_path(&closedmesh_cache_dir());
...
async fn try_unload_from_running_runtime(names: &[String]) {
    ...
    if ports.is_empty() {
        ...
        // `closedmesh models delete` while the desktop service is up but
