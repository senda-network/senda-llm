...
fn default_blobstore_root() -> PathBuf {
    crate::models::local::closedmesh_cache_dir().join("blobstore")
}
...
fn blobstore_server_info() -> ServerInfo {
    ServerInfo::new(ServerCapabilities::builder().build())
        ...
        .with_instructions(
            "Provides internal request object storage for the closedmesh host. Not intended for direct user tools.",
        )
}
