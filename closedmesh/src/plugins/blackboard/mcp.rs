...
//!
//! Run with: `closedmesh blackboard --mcp`
...
/// Base URL for the closedmesh management API.
#[allow(dead_code)]
...
impl BlackboardServer {
    ...
    async fn post(&self, Parameters(params): Parameters<PostParams>) -> String {
            ...
            .await
        {
            ...
            Err(e) => format!("Cannot reach closedmesh on port {}: {e}", self.port),
        }
    }
...
mod tests {
    ...
    async fn test_post_no_server() {
        ...
        let result = server.post(Parameters(params)).await;
        assert!(
            result.contains("Cannot reach closedmesh"),
