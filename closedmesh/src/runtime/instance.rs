...
//!
//! Each non-client closedmesh invocation acquires an `InstanceRuntime` under
...
//! pidfiles for every child process that closedmesh spawns. On startup,
//! other closedmesh instances scan the root for runtime dirs whose locks
...
//! - `lock` — `flock(2)` advisory lock file held by the owning closedmesh
...
//! this precedence:
//!
//! 1. `CLOSEDMESH_RUNTIME_ROOT` environment variable (highest; used by tests)
//! 2. `$XDG_RUNTIME_DIR/closedmesh/runtime` (systemd services, rootless containers)
...
//!   `CLOSEDMESH_RUNTIME_ROOT` to a local path in NFS environments.
//! - **Symlinked `~/.closedmesh`**: two closedmesh instances started via different
...
pub struct PidfileMetadata {
    ...
    /// Owner process PID (the closedmesh instance that spawned the child).
    pub owner_pid: u32,
...
/// Resolve the runtime root directory for this closedmesh installation.
///
/// Precedence:
/// 1. `CLOSEDMESH_RUNTIME_ROOT` environment variable (test override / custom deployment)
/// 2. `$XDG_RUNTIME_DIR/closedmesh/runtime`
...
fn runtime_root_with_home(home: Option<PathBuf>) -> Result<PathBuf> {
    ...
    if let Ok(root) = std::env::var("CLOSEDMESH_RUNTIME_ROOT") {
        return Ok(PathBuf::from(root));
    }

    // 2. XDG_RUNTIME_DIR (standard on modern Linux)
    if let Ok(xdg) = std::env::var("XDG_RUNTIME_DIR") {
        return Ok(PathBuf::from(xdg).join("closedmesh").join("runtime"));
    }
    ...
    anyhow::bail!(
        "closedmesh requires a home directory, XDG_RUNTIME_DIR, or CLOSEDMESH_RUNTIME_ROOT to be set"
    )
}

/// A scoped runtime directory for a single closedmesh process instance.
///
...
/// Snapshot of a co-located closedmesh instance discovered via the runtime root.
#[derive(Debug, Clone, Serialize)]
pub struct LocalInstanceSnapshot {
    /// PID of the closedmesh process that owns this runtime directory.
    pub pid: u32,
...
struct OwnerMetadata {
    ...
    closedmesh_binary: Option<String>,
}
...
pub(crate) fn collect_runtime_stop_targets(
        ...
        .flatten()
    {
        ...
        let expected_comm = owner
            ...
            .unwrap_or_else(|| "closedmesh".to_string());
...
/// Scan `root` for live co-located closedmesh instances.
///
...
mod scan_tests {
    ...
    fn write_owner_json(
        ...
        let meta = serde_json::json!({
            ...
            "closedmesh_binary": "/usr/bin/closedmesh",
        });
...
mod tests {
    ...
    fn runtime_root_respects_env_override() {
        ...
        let _g = EnvGuard::save_and_set("CLOSEDMESH_RUNTIME_ROOT", dir.path().to_str().unwrap());
    ...
    fn runtime_root_falls_back_to_xdg() {
        let dir = tempdir().unwrap();
        let _g_mesh = EnvGuard::save_and_remove("CLOSEDMESH_RUNTIME_ROOT");
    ...
    fn runtime_root_bails_when_unset() {
        ...
        assert!(
            msg.contains("HOME")
                ...
                || msg.contains("CLOSEDMESH_RUNTIME_ROOT"),
    ...
    async fn reap_skips_alive_owner() {
        let root = tempdir().unwrap();
        let _g = EnvGuard::save_and_set("CLOSEDMESH_RUNTIME_ROOT", root.path().to_str().unwrap());
