...
pub enum FixtureError {
    #[error("closedmesh binary not found at {0}")]
    BinaryNotFound(PathBuf),
    #[error("closedmesh startup timed out after 60s")]
    StartupTimeout,
...
impl FixtureMesh {
    pub fn new(model: &str) -> Result<Self, FixtureError> {
        let binary = find_closedmesh_binary()?;
        let port = pick_port();
...
fn find_closedmesh_binary() -> Result<PathBuf, FixtureError> {
    // Check $CLOSEDMESH_BIN env var first, then target/release/closedmesh
    if let Ok(bin) = std::env::var("CLOSEDMESH_BIN") {
