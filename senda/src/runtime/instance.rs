...
//!
//! Each non-client senda invocation acquires an `InstanceRuntime` under
//! `~/.senda/runtime/{pid}/` (overridable via env vars). The directory
//! holds an advisory `flock(2)` lock for the instance's lifetime, plus
//! pidfiles for every child process that senda spawns. On startup,
//! other senda instances scan the root for runtime dirs whose locks
...
//! - `lock` — `flock(2)` advisory lock file held by the owning senda
...
//! - Application state, configuration, or catalog caches (live elsewhere under `~/.senda/`)
//! - Unix domain sockets (out of scope — use the API port)
//! - Downloaded model files (live under `~/.senda/models/`)
...
//! this precedence:
//!
//! 1. `SENDA_RUNTIME_ROOT` environment variable (highest; used by tests)
//! 2. `$XDG_RUNTIME_DIR/senda/runtime` (systemd services, rootless containers)
...
//!   `SENDA_RUNTIME_ROOT` to a local path in NFS environments.
//! - **Symlinked `~/.senda`**: two senda instances started via different
...
pub struct PidfileMetadata {
    ...
    /// Owner process PID (the senda instance that spawned the child).
    pub owner_pid: u32,
...
/// Resolve the runtime root directory for this senda installation.
///
/// Precedence:
/// 1. `SENDA_RUNTIME_ROOT` environment variable (test override / custom deployment)
/// 2. `$XDG_RUNTIME_DIR/senda/runtime`
...
fn runtime_root_with_home(home: Option<PathBuf>) -> Result<PathBuf> {
    ...
    if let Ok(root) = std::env::var("SENDA_RUNTIME_ROOT") {
        return Ok(PathBuf::from(root));
    }

    // 2. XDG_RUNTIME_DIR (standard on modern Linux)
    if let Ok(xdg) = std::env::var("XDG_RUNTIME_DIR") {
        return Ok(PathBuf::from(xdg).join("senda").join("runtime"));
    }

    // 3. Platform home directory. On Windows this can be available even when
    // HOME is unset in the launching shell.
    if let Some(home) = home {
        return Ok(home.join(".senda").join("runtime"));
    }

    // 4. Nothing usable - fail fast with a clear message.
    anyhow::bail!(
        "senda requires a home directory, XDG_RUNTIME_DIR, or SENDA_RUNTIME_ROOT to be set"
    )
}

/// A scoped runtime directory for a single senda process instance.
///
...
/// Snapshot of a co-located senda instance discovered via the runtime root.
#[derive(Debug, Clone, Serialize)]
pub struct LocalInstanceSnapshot {
    /// PID of the senda process that owns this runtime directory.
    pub pid: u32,
...
pub(crate) fn collect_runtime_stop_targets(
        ...
        .flatten()
    {
        ...
        let expected_comm = owner
            ...
            .unwrap_or_else(|| "senda".to_string());
...
/// Scan `root` for live co-located senda instances.
///
...
mod tests {
    ...
    fn runtime_root_respects_env_override() {
        ...
        let _g = EnvGuard::save_and_set("SENDA_RUNTIME_ROOT", dir.path().to_str().unwrap());
    ...
    fn runtime_root_falls_back_to_xdg() {
        let dir = tempdir().unwrap();
        let _g_mesh = EnvGuard::save_and_remove("SENDA_RUNTIME_ROOT");
        ...
        assert_eq!(root, dir.path().join("senda").join("runtime"));
    }
    ...
    fn runtime_root_falls_back_to_windows_profile_without_home() {
        ...
        assert_eq!(root, home.join(".senda").join("runtime"));
    }
    ...
    fn runtime_root_bails_when_unset() {
        ...
        assert!(
            msg.contains("HOME")
                ...
                || msg.contains("SENDA_RUNTIME_ROOT"),
    ...
    async fn reap_skips_alive_owner() {
        let root = tempdir().unwrap();
        let _g = EnvGuard::save_and_set("SENDA_RUNTIME_ROOT", root.path().to_str().unwrap());
