...
fn blobstore_manifest() -> senda_plugin::proto::PluginManifest {
...
fn default_blobstore_root() -> PathBuf {
    crate::models::local::senda_cache_dir().join("blobstore")
}
...
fn blobstore_server_info() -> ServerInfo {
    ServerInfo::new(ServerCapabilities::builder().build())
        ...
        .with_instructions(
            "Provides internal request object storage for the senda host. Not intended for direct user tools.",
        )
}
...
fn build_blobstore_plugin(name: String) -> senda_plugin::InternalRpcPlugin {
