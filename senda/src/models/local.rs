...
pub fn senda_cache_dir() -> PathBuf {
    dirs::cache_dir()
        .unwrap_or_else(|| {
            dirs::home_dir()
...
pub fn model_metadata_cache_dir() -> PathBuf {
    senda_cache_dir().join("model-meta")
}
...
mod tests {
    ...
    fn huggingface_cache_prefers_explicit_hub_cache() {
        ...
        std::env::set_var("HF_HUB_CACHE", "/tmp/senda-hub-cache");
        std::env::set_var("HF_HOME", "/tmp/senda-hf-home");
        std::env::set_var("XDG_CACHE_HOME", "/tmp/senda-xdg");

        assert_eq!(
            huggingface_hub_cache_dir(),
            PathBuf::from("/tmp/senda-hub-cache")
        );
    ...
    fn huggingface_cache_falls_back_to_hf_home() {
        ...
        assert_eq!(
            huggingface_hub_cache_dir(),
            PathBuf::from("/tmp/senda-hf-home").join("hub")
        );
    ...
    fn mmproj_path_prefers_quant_matched_generic_sibling() {
        ...
        let temp = std::env::temp_dir().join(format!(
            "senda-mmproj-quant-sibling-test-{}",
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
                .as_nanos()
