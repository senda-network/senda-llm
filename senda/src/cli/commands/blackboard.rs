...
pub(crate) async fn run_blackboard(
    ...
    if status_resp.is_err() {
        eprintln!("No senda node running on port {port}.");
        eprintln!();
        eprintln!("Blackboard requires a running mesh node:");
        eprintln!("  Private mesh:  senda client  (share the join token printed out)");
    ...
    if let Some(msg) = text {
        ...
        let resp = client
            ...
            .send()
            .await
            .context("Cannot reach senda — is it running?")?;
