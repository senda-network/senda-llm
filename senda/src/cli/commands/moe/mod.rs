...
fn print_submit_suggestion(model_path: &Path) {
    ...
    println!("📤 Contribute this ranking to senda so other users can reuse it:");
    println!("  senda moe share '{}'", identity.distribution_ref());
}
...
async fn run_share_resolved(
    ...
    let token = models::hf_token_override().ok_or_else(|| {
        share_error(
            ...
            "Set HF_TOKEN or HUGGING_FACE_HUB_TOKEN before running `senda moe share`.",
        )
    })?;
...
fn resolve_analyze_binary() -> Result<PathBuf> {
    ...
    let names = [
        format!("llama-moe-analyze{exe_suffix}"),
