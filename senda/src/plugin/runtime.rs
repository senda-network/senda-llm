...
impl ExternalPlugin {
    ...
    async fn ensure_running(&self) -> Result<()> {
        ...
        child.env("SENDA_PLUGIN_ENDPOINT", &endpoint);
        child.env("SENDA_PLUGIN_TRANSPORT", transport);
        child.env("SENDA_PLUGIN_NAME", &self.spec.name);
        if let Some(ref url) = self.spec.url {
            child.env("SENDA_OPENAI_ENDPOINT_URL", url);
        }
