    ...
    if let Some(release) = latest_release_info().await {
        ...
        match bundle_asset {
            Some(ref asset) if release.assets.iter().any(|a| a == asset) => {
                eprintln!(
                    "✨ New version: v{VERSION} -> v{}. Run 'senda update'.",
                    release.version
                );
            }
            _ => {
...
pub(crate) async fn run_update_command(cli: &Cli) -> Result<()> {
    ...
    if requested_version.is_none() && !version_newer(&release.version, VERSION) {
        eprintln!("senda is already up to date (v{VERSION}).");
        return Ok(());
    }
...
fn require_update_target(cli: &Cli) -> Result<UpdateTarget> {
    if !platform_has_release_assets() {
        bail!(
            "`senda update` is not supported on this platform. Download the latest release from {RELEASES_URL}."
        );
    }

    let exe = std::env::current_exe().context("Cannot determine senda executable path")?;
    let Some((install_dir, bundle_flavor)) = bundle_install_dir(&exe, cli.llama_flavor) else {
        bail!(
            "`senda update` only works for release-bundle installs. Current executable: {}",
            exe.display()
        );
    };
...
fn mesh_binary_name() -> String {
    launch::platform_bin_name("senda")
}
...
async fn install_latest_bundle(
    ...
    let workspace = install_dir.join(format!(".senda-update-{unique}"));
...
mod zip_tests {
    ...
    fn extract_zip_archive_strips_top_level_directory() -> Result<()> {
        let base_dir = unique_temp_dir("senda_extract_zip_test");
...
try {{
    ...
    if ($restartAfterUpdate) {{
        $env:SENDA_SELF_UPDATE_ATTEMPTED = '1'
        & $exePath @args
        exit $LASTEXITCODE
    }}
    exit 0
...
fn exec_current_binary(exe: &Path) -> Result<()> {
    ...
    Err(errno).context("Failed to restart updated senda")
}
...
mod tests {
    ...
    fn test_release_repo_can_be_overridden_for_testing() {
        std::env::set_var(SELF_UPDATE_REPO_ENV, "jdumay/senda");
        assert_eq!(release_repo(), "jdumay/senda");
        assert_eq!(
            latest_release_api_url(),
            "https://api.github.com/repos/jdumay/senda/releases/latest"
        );
        assert_eq!(
            release_api_url_for_tag("v0.60.0"),
            "https://api.github.com/repos/jdumay/senda/releases/tags/v0.60.0"
        );
        assert_eq!(
            release_asset_url("v0.60.0", "senda-linux-x86_64.tar.gz"),
            "https://github.com/jdumay/senda/releases/download/v0.60.0/senda-linux-x86_64.tar.gz"
        );
        std::env::remove_var(SELF_UPDATE_REPO_ENV);
    }
    ...
    fn test_linux_arm64_release_asset_names() {
        let stable_asset = "senda-linux-aarch64.tar.gz".to_string();
    ...
    fn test_resolve_release_asset_name_prefers_stable_linux_arm64_asset() {
        let release = ReleaseInfo {
            ...
            version: "0.60.0".to_string(),
            assets: vec![
                "senda-linux-aarch64.tar.gz".to_string(),
                "senda-v0.60.0-linux-aarch64.tar.gz".to_string(),
            ],
        };
    ...
    fn test_resolve_release_asset_name_falls_back_to_versioned_linux_arm64_asset() {
        let release = ReleaseInfo {
            tag: "v0.60.0".to_string(),
            version: "0.60.0".to_string(),
            assets: vec!["senda-v0.60.0-linux-aarch64.tar.gz".to_string()],
        };
        ...
        assert_eq!(
            ...
            Some("senda-v0.60.0-linux-aarch64.tar.gz".to_string())
        );
    }
    ...
    fn test_resolve_release_asset_name_versioned_first_falls_back_to_stable() {
        let release = ReleaseInfo {
            tag: "v0.60.0".to_string(),
            version: "0.60.0".to_string(),
            assets: vec!["senda-linux-aarch64.tar.gz".to_string()],
        };
    ...
    fn test_should_attempt_auto_update_only_when_flag_is_set() {
        ...
        let cli = Cli::parse_from(["senda", "--auto-update", "--auto"]);
        assert!(should_attempt_auto_update(&cli));

        let cli = Cli::parse_from(["senda", "download"]);
        assert!(!should_attempt_auto_update(&cli));

        let cli = Cli::parse_from(["senda", "--auto-update", "update"]);
        assert!(!should_attempt_auto_update(&cli));
    }
