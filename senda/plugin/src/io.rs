...
pub async fn connect_from_env() -> Result<LocalStream> {
    let endpoint = std::env::var("SENDA_PLUGIN_ENDPOINT")
        .context("SENDA_PLUGIN_ENDPOINT is not set for plugin process")?;
    let transport =
        std::env::var("SENDA_PLUGIN_TRANSPORT").unwrap_or_else(|_| default_transport().into());
