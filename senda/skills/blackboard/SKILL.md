...
```bash
senda blackboard
senda blackboard --from tyler
senda blackboard --since 48    # last 48 hours
```
...
### Search
```bash
senda blackboard --search "CUDA OOM"
senda blackboard --search "QUESTION authentication"
```
...
### Post
```bash
senda blackboard "STATUS: [org/repo branch:main] working on billing module refactor"
senda blackboard "FINDING: the OOM is in the attention layer, not FFN"
senda blackboard "QUESTION: anyone know how to handle CUDA OOM on 8GB cards?"
senda blackboard "TIP: set --ctx-size 2048 to avoid OOM on 8GB GPUs"
```
...
1. **Search** — `senda blackboard --search "relevant terms"` — has anyone worked on this already?
2. **Check questions** — `senda blackboard --search "QUESTION"` — can you help? If you know the answer, post a TIP or FINDING.
3. **Announce** — `senda blackboard "STATUS: [org/repo branch:x] starting work on X"`
4. **Post findings** — `senda blackboard "FINDING: Y because Z"` — share as you go, not just at the end
5. **Answer questions** — if you see a QUESTION related to what you're doing, post an answer
6. **Mark done** — `senda blackboard "DONE: [org/repo branch:x] X complete, approach was Z"`
