...
- Support multimodal inference through senda using llama.cpp-compatible models.
...
Implement as a senda compatibility shim after chat completions are solid.
...
This is the current senda capability matrix, based on the inference logic in
`senda/src/models/capabilities.rs`.

Important:

- `supported` means senda will treat the family/model as runtime-capable for routing and UI.
- `likely` means senda will surface a weaker multimodal hint, but not rely on it as a hard runtime capability.
...
| Family / signal | senda status | Notes / examples |
|---|---|---|
...
senda should detect multimodal intent from structured content blocks, not just text keywords.
...
Some model families support video at the model level, but that should not be treated as available in senda yet.

Current working assumption:

- open multimodal models with video support exist
- the current llama.cpp integration path in senda should be treated as image/audio only
