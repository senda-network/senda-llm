...
Today senda has two separate identity concepts:
...
- `senda auth init` creates an owner keypair in the keystore
...
`~/.senda/key` or an ephemeral key for clients. That node identity is what
...
- **Owner identity**
  ...
  - This is the existing keystore identity created by `senda auth init`
...
This is the right split for senda:
...
- `senda/src/crypto/ownership.rs`
  - `NodeOwnershipClaim`
  - `SignedNodeOwnership`
  - canonical serialization for signing
  - sign/verify helpers
- `senda/src/crypto/mod.rs`
...
```bash
senda auth renew-node
senda auth rotate-node
senda auth rotate-node --revoke-current
senda auth rotate-owner
```
...
```bash
senda --owner-key ~/.senda/owner-keystore.json
senda --owner-required
senda --owner-label studio-mac
senda --trust-owner <owner_id>
senda --trust-policy off|prefer-owned|require-owned|allowlist
```
...
senda auth status
senda auth renew-node
senda auth sign-node --node-key ~/.senda/key --out ~/.senda/node-ownership.json
senda auth verify-node --file ~/.senda/node-ownership.json
senda auth rotate-node
senda auth rotate-node --revoke-current
senda auth revoke-owner <owner_id>
senda auth revoke-node --cert-id <cert_id>
senda auth revoke-node --node-id <node_endpoint_id>
senda auth rotate-owner
senda auth trust add <owner_id>
senda auth trust list
senda auth trust remove <owner_id>
```
...
senda --model Qwen3-14B --owner-key ~/.senda/owner-keystore.json --owner-label studio

senda --join <TOKEN> --owner-key ~/.senda/owner-keystore.json --owner-label mini
```

Strict private mesh:

```bash
senda --model Qwen3-30B \
  --owner-key ~/.senda/owner-keystore.json \
...
That gives senda a durable answer to:
